...
function SystemCore({ zone, index, active, onHover }) {
  const ref = useRef(), pulseSpeed = .72 + index * .11
  ...
  return <group position={zone.pos}>
    <group ref={ref}>
      <mesh onPointerOver={e => { e.stopPropagation(); onHover(index) }} onPointerOut={() => onHover(null)}><icosahedronGeometry args={[.25, 1]} /><meshBasicMaterial color={zone.color} wireframe /></mesh>
      <mesh><icosahedronGeometry args={[.12, 1]} /><meshBasicMaterial color="white" /></mesh>
      <EnergyRings color={zone.color} radius={.36} speed={pulseSpeed} intensity={active ? 1.35 : .9} />
      <pointLight color={zone.color} intensity={active ? 5 : 2.2} distance={3.2} />
    </group>
    <Html position={[index % 2 ? -1.08 : .72, index % 3 === 0 ? .72 : -.72, 0]} distanceFactor={11} className={`system-label ${active ? 'active' : ''}`}>
      <div><strong>{LABELS[index]}</strong><span>{index === 7 ? 'READY' : 'ACTIVE'}</span></div><i />
    </Html>
  </group>
}
...
export default function NeuralNetwork({ ready, boot }) {
  const group = useRef(), points = useRef(), [hovered, setHovered] = [useRef(null), value => { hovered.current = value }]
  const data = useMemo(generateArchitecture, []), { pointer } = useThree(), { state, sequence } = useNeuralState()
  ...
  useFrame(({ clock }, delta) => {
    ...
    points.current.material.opacity = THREE.MathUtils.lerp(points.current.material.opacity, ready ? .96 : booting ? .78 : .01, .035)
