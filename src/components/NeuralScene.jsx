...
export default function NeuralScene({ ready }) {
  return <div className={`scene ${ready ? 'online' : ''}`}>
    <Canvas dpr={[1, 1.65]} camera={{ position: [0, 0, 14.5], fov: 48 }} gl={{ antialias: false, powerPreference: 'high-performance' }}>
      <color attach="background" args={['#010208']} />
      <fog attach="fog" args={['#010208', 13, 29]} />
      <ambientLight intensity={0.08} />
      <Suspense fallback={null}><NeuralNetwork ready={ready} /></Suspense>
      <Stars radius={35} depth={18} count={CONFIG.starCount} factor={1.7} saturation={0.35} fade speed={0.22} />
      <OrbitControls enablePan={false} minDistance={10.5} maxDistance={19} rotateSpeed={0.32} zoomSpeed={0.4} dampingFactor={0.055} enableDamping />
